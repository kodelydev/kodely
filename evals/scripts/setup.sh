...
build_extension() {
  ...
  npx vsce package --out bin/kodely-latest.vsix || exit 1
  code --install-extension bin/kodely-latest.vsix || exit 1
  cd evals
}
...
code --install-extension kodely.Kodely &>/dev/null || exit 1
...
if [[ ! -s "../bin/kodely-latest.vsix" ]]; then
  build_extension
else
