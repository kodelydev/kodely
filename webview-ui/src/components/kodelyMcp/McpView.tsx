...
const ServerRow = ({ server }: { server: McpServer }) => {
	const { mcpMarketplaceCatalog, autoApprovalEnabled } = useExtensionState()
	...
	const handleAutoApproveChange = () => {
		...
		vscode.postMessage({
			...
			autoApprove: !server.tools?.every((tool) => tool.alwaysAllow),
		})
	}
							...
							<VSCodePanelView id="tools-view">
								{server.tools && server.tools.length > 0 ? (
									<div
										...
										{server.name && autoApprovalEnabled && (
											<VSCodeCheckbox
												style={{ marginBottom: -10 }}
												checked={server.tools.every((tool) => tool.alwaysAllow)}
